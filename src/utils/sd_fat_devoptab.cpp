...
typedef struct _sd_fat_file_state_t {
    ...
    uint64_t pos;                                    /* Current position within the file (in bytes) */
    uint64_t len;                                    /* Total length of the file (in bytes) */
...
static int sd_fat_open_r (struct _reent *r, void *fileStruct, const char *path, int flags, int mode) {
    ...
    OSLockMutex((OSMutex *)dev->pMutex);

    char *real_path = sd_fat_real_path(path, dev);
    if(!path) {
        r->_errno = ENOMEM;
        OSUnlockMutex((OSMutex *)dev->pMutex);
        return -1;
    }
...
static int sd_fat_dirnext_r (struct _reent *r, DIR_ITER *dirState, char *filename, struct stat *st) {
    ...
    FSDirectoryEntry * dir_entry = (FSDirectoryEntry *)malloc(sizeof(FSDirectoryEntry));
...
static int sd_fat_add_device (const char *name, const char *mount_path, FSClient *pClient, FSCmdBlock *pCmd) {
    devoptab_t *dev = NULL;
    char *devname = NULL;
    char *devpath = NULL;
    int i;
    ...
    priv->pCmd = pCmd;
    priv->pMutex = (OSMutex*) malloc(sizeof(OSMutex));
...
void deleteDevTabsNames() {
    const devoptab_t * devoptab = NULL;
    uint32_t last_entry = (uint32_t) devoptab_list[STD_MAX-1];
    for (int i = 3; i < STD_MAX; i++) {
        devoptab = devoptab_list[i];

        if (devoptab) {
            //log_printf("check devotab %d %08X\n",i,devoptab);
            if((uint32_t) devoptab != last_entry) {
...
static int sd_fat_remove_device (const char *path, FSClient **pClient, FSCmdBlock **pCmd, char **mountPath) {
    const devoptab_t *devoptab = NULL;
    char name[128] = {0};
    int i;
...
int32_t mount_sd_fat(const char *path) {
    ...
    FSInit();
    FSInitCmdBlock(pCmd);
    FSAddClient(pClient, 0);

    char *mountPath = NULL;

    DEBUG_FUNCTION_LINE("Calling MountFS\n");
